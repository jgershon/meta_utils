...
template <typename... T>
struct is_tuple_<tuple<T...>> : std::true_type
{};
...
struct is_predicate_<T, tuple<Elements...>> : std::true_type
{};
...
struct contains_<tuple<T1...>, T2> : std::true_type
{};
...
struct cat_<tuple<T1...>, tuple<T2...>, T3...>
    : cat_<tuple<T1..., T2...>, typename cat_<T3...>::type>
{};
...
{
    using type = tuple<std::tuple_element_t<I, Tuple>...>;
};
...
export {
    ...
    template <tuple_r... Tuples>
    using cat = typename cat_<Tuples...>::type;
    ...
    using insert = cat<head<I, Tuple>, tuple<T>, tail<size<Tuple> - I, Tuple>>;
...
template <typename... T, predicate_r<tuple<T...>> Pred, bool Criterion>
struct where_<tuple<T...>, Pred, Criterion>
{
    using type =
...
template <typename... T, typename V, bool Criterion>
    requires(!predicate_r<V, tuple<T...>>)
struct where_<tuple<T...>, V, Criterion>
{
    struct pred
    {
    ...
    using type = typename where_<tuple<T...>, pred, Criterion>::type;
};
...
struct unique_<Tuple, Unique>
    : unique_<pop_front<Tuple>, cat<Unique, head<0, Tuple>>>
{};
