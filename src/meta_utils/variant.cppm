...
template <typename... T>
struct is_variant_<variant<T...>> : std::true_type
{};
...
{
    using type = variant<V...>;
};

template <typename... V, typename... T>
struct make_variant_<std::tuple<V...>, void, T...>
    : make_variant_<std::tuple<V...>, monostate, T...>
{};

template <typename... V, typename... T>
    requires(!tuple::contains_r<std::tuple<V...>, monostate>)
struct make_variant_<std::tuple<V...>, monostate, T...>
    : make_variant_<std::tuple<monostate, V...>, T...>
{};
...
struct visit_result_<Func, variant<Args...>>
...
export {
    ...
    using insert = unite<Var, variant<T>>;

    template <typename T, variant_r Var>
    using erase = diff<Var, variant<T>>;
